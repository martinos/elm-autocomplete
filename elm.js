...
Elm.Helper.make = function (_elm) {
   ...
   var tests = function () {
      ...
      _U.list([A2($ElmTest.test,
              ...
              A2($ElmTest.assertEqual,A3(changeAt,10,$Basics.always(100),orig),$Array.fromList(_U.list([1,2]))))
              ,A2($ElmTest.test,
              "changeAt an existing value",
              A2($ElmTest.assertEqual,A3(changeAt,0,$Basics.always(100),orig),$Array.fromList(_U.list([100,2]))))]));
   }();
   var main = $ElmTest.elementRunner(tests);
   return _elm.Helper.values = {_op: _op,changeAt: changeAt,tests: tests,main: main};
};
...
Elm.Selector.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var withIndex = $Array.indexedMap(F2(function (v0,v1) {    return {ctor: "_Tuple2",_0: v0,_1: v1};}));
   var toggle = function (elem) {    return _U.update(elem,{selected: $Basics.not(elem.selected)});};
   var select = function (elem) {    return _U.update(elem,{selected: true});};
   var unselect = function (elem) {    return _U.update(elem,{selected: false});};
   ...
   var navigate = F4(function (step,start_id,index,items) {
      ...
      if (_p0.ctor === "Nothing") {
            ...
            if (_p1.ctor === "Nothing") {
                  ...
                  return A3($Helper.changeAt,step(_p2),select,resetSelections(items));
               }
         }
   });
   ...
   var selectedItemIndex = function (items) {
      ...
      A2($Array.get,0,A2($Array.filter,function (_p4) {    return function (_) {    return _.selected;}($Basics.snd(_p4));},withIndex(items))));
   };
   ...
   var update = F2(function (action,list) {    return $Array.toList(A2($Debug.log,"list",A2(updateWithIds,action,$Array.fromList(list))));});
   var NoOp = {ctor: "NoOp"};
   var Next = {ctor: "Next"};
   var Prev = {ctor: "Prev"};
...
Elm.Main.make = function (_elm) {
   ...
   var onlySelected = F2(function (name,list) {
      return _U.eq($List.length(A2($List.filter,function (_) {    return _.selected;},list)),
      $List.length(A2($List.filter,function (r) {    return _U.eq(r,{name: name,selected: true});},list)));
   });
   var selected = function (name) {    return $List.member({name: name,selected: true});};
   var lastSelected = _U.list([{name: "Martin",selected: true},{name: "Joe",selected: false}]);
   var firstSelected = _U.list([{name: "Martin",selected: true},{name: "Joe",selected: false}]);
   var noneSelected = _U.list([{name: "Martin",selected: false},{name: "Joe",selected: false}]);
   var tests = A2($ElmTest.suite,
   "update",
   _U.list([A2($ElmTest.suite,
           "Next",
           _U.list([A2($ElmTest.test,
                   "if no element selected it selects the first",
                   $ElmTest.assert(A2(onlySelected,"Martin",A2($Selector.update,$Selector.Next,noneSelected))))
                   ,A2($ElmTest.test,
                   "it selects only the next element",
                   $ElmTest.assert(A2(onlySelected,"Joe",A2($Selector.update,$Selector.Next,firstSelected))))
                   ,A2($ElmTest.test,
                   "if last is selected, it stays selected",
                   $ElmTest.assert(A2(onlySelected,"Joe",A2($Selector.update,$Selector.Next,lastSelected))))]))
           ,A2($ElmTest.suite,
           "Prev",
           _U.list([A2($ElmTest.test,
                   "if no element selected, it selects the last element",
                   $ElmTest.assert(A2(onlySelected,"Joe",A2($Selector.update,$Selector.Prev,noneSelected))))
                   ,A2($ElmTest.test,
                   "it selects the previous element",
                   $ElmTest.assert(A2(onlySelected,"Martin",A2($Selector.update,$Selector.Prev,lastSelected))))
                   ,A2($ElmTest.test,
                   "If first selected it stays selected",
                   $ElmTest.assert(A2(onlySelected,"Martin",A2($Selector.update,$Selector.Prev,firstSelected))))]))]));
   var main = $ElmTest.elementRunner(tests);
   var Item = F2(function (a,b) {    return {name: a,selected: b};});
